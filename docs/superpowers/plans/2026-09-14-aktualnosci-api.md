...
5. **Wymagana poprawka kodu przed pierwszym realnym uploadem zdjęcia w produkcji** (znalezisko z finalnego review, świadomie nie naprawione w tym planie — patrz ledger): `app.UseForwardedHeaders(...)` w `Program.cs` ufa domyślnie tylko loopbackowi, więc na Railway `Request.Scheme` zostanie `http`, a zapisane `CoverImageUrl` będą miały złe schema (mixed content na https). Dodaj do tego samego bloku `ForwardedHeadersOptions`: `options.KnownNetworks.Clear(); options.KnownProxies.Clear();` (bezpieczne tu, bo jedyny proxy widoczny z kontenera Railway to jego własny edge). Zweryfikuj pierwszym realnym uploadem — zwrócony URL powinien zaczynać się od `https://`.
